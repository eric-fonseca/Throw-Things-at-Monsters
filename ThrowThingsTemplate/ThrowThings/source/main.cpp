...
void TestGame::respawnEnemies()
{
	...
	// set up enemies
	Enemy* enemy1 = new Enemy("", 1, 75, p, splatTex);
	for(int i = 0; i < 7; i++)
	{
		int rX = rand() % 800;
		int rY = rand() % 600;
		...
		enemy1 = new Enemy("", 1, 75, p, splatTex);
	}

	for(int i = 0; i < 2; i++)
	{
		Brute* brute1 = new Brute(p, splatTex);
		
		int rX = rand() % 800;
		int rY = rand() % 600;
		brute1->SetPosition(Vector2(rX, rY));
		brute1->SetTexture(bruteTex);
		brute1->SetOrigin(Vector2(16, 16));
		enemies.push_back(brute1);
	}

	Volatile* volatile1 = new Volatile(p, splatTex);

	int rX = rand() % 800;
	int rY = rand() % 600;
	volatile1->SetPosition(Vector2(rX, rY));
	volatile1->SetTexture(volatileTex);
	volatile1->SetOrigin(Vector2(16, 16));
	enemies.push_back(volatile1);
}
...
void TestGame::VOnStartup(void)
{
	...
	bruteTex = m_content.LoadTexture(VTEXT("brute.png"));
	volatileTex = m_content.LoadTexture(VTEXT("volatile.png"));
...
void TestGame::VOnRender(float dt)
{
	...
	GLRENDERER->Render2DTexture((GLTexture*)barOuterTex, Vector2(20, m_window->VGetClientBounds().h - 80),
		Rect::EMPTY, Vector2::Zero, Vector2::Unit, 0.0f, 1.0f, Colors::White, 0.0f);
	GLRENDERER->Render2DTexture((GLTexture*)barInnerTex, Vector2(20, m_window->VGetClientBounds().h - 80),
